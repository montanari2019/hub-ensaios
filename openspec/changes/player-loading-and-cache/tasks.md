...
- [ ] 7.4 Verify in the browser (throttle or block one channel's request via devtools, or reproduce with a short test timeout) that a stalled channel now shows the player's existing error screen within a bounded time instead of leaving the loading percentage frozen.
